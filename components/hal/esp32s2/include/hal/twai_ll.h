...
static inline void twai_ll_parse_frame_header(const twai_ll_frame_buffer_t *rx_frame, twai_frame_header_t *header)
{
    ...
    header->fdf = 0;  // Classic CAN doesn't support FD
    header->brs = 0;  // Classic CAN doesn't support BRS
    header->esi = 0;  // Classic CAN doesn't support ESI
    header->timestamp = 0;  // Classic CAN doesn't have timestamp
