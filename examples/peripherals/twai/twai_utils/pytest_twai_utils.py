...
class TestConfig:
    ...
    # Basic filter tests
    BASIC_FILTER_TESTS = [
        ('', [('123#DEAD', 0x123, True), ('456#BEEF', 0x456, True)]),  # No filter
        ('123:7FF', [('123#DEAD', 0x123, True), ('456#BEEF', 0x456, False)]),  # Exact match
    ]

    # FD range filter tests
    FD_RANGE_FILTER_TESTS = [
        (
...
class TwaiTestHelper:
    ...
    def expect(self, patterns: Union[List[str], str], timeout: Optional[float] = None) -> bool:
    ...
    def run(self, cmd: str, expect: Optional[Union[List[str], str]] = None, timeout: Optional[float] = None) -> bool:
        self.sendline(cmd)
    ...
    def build_init(
        ...
        *,
        controller_id: int = 0,
        tx_gpio: Optional[int] = None,
        rx_gpio: Optional[int] = None,
        bitrate: Optional[int] = None,
        clk_out_gpio: Optional[int] = None,
        bus_off_gpio: Optional[int] = None,
    ...
    def build_dump_start(self, *, controller_id: int = 0, dump_filter: Optional[str] = None) -> str:
    ...
    def dump_start(self, controller_id: int = 0, dump_filter: Optional[str] = None) -> bool:
    ...
    def recover(self, controller_id: int = 0, timeout_ms: Optional[int] = None) -> bool:
...
def test_twai_basic_operations(twai: TwaiTestHelper) -> None:
    with twai.session(mode='standard', tx_gpio=TestConfig.DEFAULT_TX_GPIO, rx_gpio=TestConfig.DEFAULT_RX_GPIO):
        assert twai.send('123#DEADBEEF'), 'Basic operation failed'
...
@pytest.mark.generic
@idf_parametrize('target', soc_filtered_targets('SOC_TWAI_SUPPORTED == 1'), indirect=['target'])
def test_twai_basic_filtering(twai: TwaiTestHelper) -> None:
    for filter_str, test_frames in TestConfig.BASIC_FILTER_TESTS:
        with twai.session(dump_filter=filter_str):
            for frame_str, expected_id, should_receive in test_frames:
                ...
                assert received is should_receive, (
                    f'Filter "{filter_str}": frame {frame_str} expected '
                    f'{"receive" if should_receive else "filter out"}, got '
                    f'{"received" if received else "filtered out"}'
                )


@pytest.mark.generic
@idf_parametrize('target', soc_filtered_targets('SOC_TWAI_SUPPORT_FD == 1'), indirect=['target'])
def test_twai_fd_range_filters(twai: TwaiTestHelper) -> None:
    for filter_str, test_frames in TestConfig.FD_RANGE_FILTER_TESTS:
        with twai.session(dump_filter=filter_str):
            ...
            if failed_cases:
                pytest.fail(f'FD range filter failed for filter "{filter_str}":\n' + '\n'.join(failed_cases))
